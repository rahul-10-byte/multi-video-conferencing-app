...
4. Runs FFmpeg to build the final MP4 (default **dynamic layout**):
   ...
   - Before stitching slices, each interval MP4 is **ffprobe**’d; missing **audio** or **video** is padded (silent AAC / black 1280×720) so FFmpeg’s `concat=v:a` filter always sees both stream types (avoids `matches no streams` / exit 234).
